...
namespace Zkey {
    ...
    PilFflonkZkeyHeader* PilFflonkZkeyHeader::loadPilFflonkZkeyHeader(BinFileUtils::BinFile *f) {
        ...
        pilFflonkZkeyHeader->X2 = f->read(fflonkZkeyHeader->n8q * 4);
        ...
        for(u_int32_t i = 0; i < lenCommits; ++i) {
            std::string name = // TODO: READ String
            pilFflonkZkeyHeader->committedF[name] = f->read(fflonkZkeyHeader->n8q * 2);
        }
        ...
        for(u_int32_t i = 0; i < lenPolsMap; ++i) {
           std::string name = // TODO: READ String
            ...
            for(u_int32_t i = 0; i < lenSubkeys; ++i) {
                u_int32_t index = f->readU32LE();
                pilFflonkZkeyHeader->polsMap[name][index] = // TODO: READ String
            }
        }
        ...
        const lenPolsNamesStage = f->readULE32();
